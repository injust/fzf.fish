...
DESCRIPTION
    ...
    can be configured using a namesake corresponding option:
        ...
        Search Git Log     |  Ctrl+Alt+L (L for log)       |  --git-log
        Search Git Status  |  Ctrl+Alt+S (S for status)    |  --git-status
...
EXAMPLES
    ...
    An agglomeration of different options
        \$ fzf_configure_bindings --git-status=ctrl-g --history=ctrl-h --variables= --processes=
